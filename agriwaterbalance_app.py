...
def produce_irrigation_calendar(results_df):
    ...
    html += '<tr><th colspan="7" style="text-align: center;">{}</th></tr>'.format(calendar.month_name[month] + " " + str(year))
    html += '<tr>'
    for day in ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]:
        html += f'<th style="border: 1px solid black; padding: 5px;">{day}</th>'
    html += '</tr>'
        ...
        for day in week:
            if day == 0:
                ...
                try:
                    ...
                    continue
                if date in results_df["Date"].values:
                    row = results_df[results_df["Date"] == date].iloc[0]
                    eta = row["ETa (mm)"]
                    et0 = row["ET0 (mm)"]
                    dr_end = row["Dr_end (mm)"]
                    raw = row["RAW (mm)"]
                    irrig_rec = round(dr_end - raw, 1) if dr_end > raw else 0
                    html += f'<td style="border: 1px solid black; padding: 5px;">{day}<br>ETa: {eta:.1f}<br>ET0: {et0:.1f}<br>Irrig: {irrig_rec:.1f}</td>'
                else:
                    html += f'<td style="border: 1px solid black; padding: 5px;">{day}</td>'
        html += '</tr>'
    html += '</table>'
