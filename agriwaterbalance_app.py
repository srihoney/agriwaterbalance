...
CROP_DATABASE = {
    ...
    # New orchard/perennial crops:
    "Almond":   {"Kc_mid":1.15, "Kc_end":0.65, "Kcb_mid":1.10, "Kcb_end":0.60, "total_days_default":180},
    "Pistachio":{"Kc_mid":1.15, "Kc_end":0.55, "Kcb_mid":1.10, "Kcb_end":0.50, "total_days_default":200},
    "Walnut":   {"Kc_mid":1.20, "Kc_end":0.70, "Kcb_mid":1.15, "Kcb_end":0.65, "total_days_default":200},
    "Citrus":   {"Kc_mid":1.00, "Kc_end":1.00, "Kcb_mid":0.95, "Kcb_end":0.95, "total_days_default":250},
    "Olive":    {"Kc_mid":0.75, "Kc_end":0.65, "Kcb_mid":0.70, "Kcb_end":0.60, "total_days_default":180}
}
...
def fetch_historical_nasa(lat, lon, start_date, end_date):
    """
    Use NASA-POWER for historical data. Return a DF with [Date, ET0, Precip, Irrigation].
    """
...
def simulate_SIMdualKc(weather_df, crop_stages_df, soil_df,
    ...
    Produces columns Dr_end (mm), RAW (mm), ETa (mm), etc.
    """
    if weather_df.empty:
...
with setup_tab:
    ...
    if run_button:
        ...
        # Step 1) Possibly fetch next 5-day forecast, storing in forecast_5days_df
        if enable_forecast:
            f5df = fetch_5day_forecast(lat_, lon_)
            if f5df.empty:
                st.warning("No 5-day forecast found or lat/lon invalid.")
            else:
                # **Compute ETa** using the chosen crop's Kc_mid
                # So that the user sees difference between ETo & ETa for next 5 days
                kc_mid = CROP_DATABASE[selected_crop]["Kc_mid"]
                f5df["ETa"] = f5df["ETo"] * kc_mid
                st.session_state.forecast_5days_df = f5df
        
        # Step 2) Build or load main weather df for the water balance
        if weather_file is not None:
            ...
            try:
                ...
                # can't parse => maybe fallback to NASA + forecast
        ...
        # Step 3) If wdf is empty => no water-balance possible
        if wdf is None or wdf.empty:
            st.error("No valid weather data => cannot run water-balance.")
            st.stop()

        # Step 4) Build or load stage df
        ...
        # Step 6) Run the water balance simulation
...
with results_tab:
    if not st.session_state.get("simulation_done", False):
        ...
        if st.session_state.results_df is None or st.session_state.results_df.empty:
            ...
            # Show a separate table for the next 5 days forecast, with ETa= ETo*Kc_mid
            st.markdown("## Next 5 Days Forecast (ETo & ETa)")
            f5_df = st.session_state.forecast_5days_df.copy()
            if not f5_df.empty:
                # We'll show columns: [Date, Tmax, Tmin, ETo, ETa]
                # We already computed ETa= ETo*Kc_mid in setup.
                # So the user sees the difference between ETo & ETa.
                columns_to_show = ["Date", "Tmax", "Tmin", "ETo", "ETa"]
                f5_df_display = f5_df[columns_to_show].copy()
                st.dataframe(f5_df_display)
                st.download_button(
                    "Download Next 5 Days Forecast (ETo, ETa)",
